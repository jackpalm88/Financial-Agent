...
class MT5ExecutionBridge:
    ...
    async def validate_signal(self, signal: Signal) -> tuple[bool, str]:
        ...
        if spread_points > self.max_spread_points:
            return False, (
                f"Spread {spread_points:.1f} points exceeds maximum "
                f"{self.max_spread_points}"
            )
        ...
        if signal.stop_loss is not None:
            ...
            if sl_distance_points < symbol_info.min_stop_distance:
                return False, (
                    "Stop loss too close: "
                    f"{sl_distance_points:.1f} < {symbol_info.min_stop_distance} points"
                )
        ...
        if signal.take_profit is not None:
            ...
            if tp_distance_points < symbol_info.min_stop_distance:
                return False, (
                    ...
                    f"{tp_distance_points:.1f} < {symbol_info.min_stop_distance} points"
                )
    ...
    async def execute_order(self, signal_id: str, signal: Signal) -> ExecutionResult:
        ...
        try:
            ...
            if order_result.success:
                ...
                slippage_display = f"{slippage_pips:.2f}" if slippage_pips is not None else "0.00"
                logger.info(
                    f"Signal {signal_id} executed: Order={order_result.order_id}, "
                    f"Fill={order_result.fill_price}, Slippage={slippage_display} pips"
                )
            else:
                exec_result = ExecutionResult(
                    success=False,
